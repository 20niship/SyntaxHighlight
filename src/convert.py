...
class SyntaxHilighter:
    def __init__(self, input_fname, output_fname, template_html_fname, language="", inline_css_=False):
        #settings
        self.program_filename = input_fname
        self.save_filename = output_fname #"test_output.html"
        self.template_html_filename = template_html_fname # "template.html"
        self.Color_XML_filename = os.path.dirname(os.path.abspath(__file__)) + "/color_setting.xml"
        self.language = language # "cpp"
        self.string_color_dic = {}
        self.inline_css = inline_css_
        ...
        while(1):
            ...
            for index, one_word in enumerate(self._text_list_oneLine):
                ...
                if self.inline_css: 
                    ...
                    self.program_after += self._setColor_oneWord(one_word)
    ...
    def LoadColorSetting(self):
        ...
        tree = ET.parse(self.Color_XML_filename)
        root = tree.getroot()
    ...
    def getLanguage(self):
        ...
        if self.language in extention_language_dic.keys():
            print("[INFO] 言語はすでに設定されています", self.language)
            return ""
